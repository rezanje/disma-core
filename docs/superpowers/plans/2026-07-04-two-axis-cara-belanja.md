> **CORRECTED 2026-07-04 (post-review, executed directly in main thread):** This plan was written before the pocket-work grab-bag session removed the entire "Transfer Vendor (Dibayar Finance)" manual queue from `finance/online-purchase.tsx` (recordVendorTransferBulk, isTransferPaid, the queue UI — all deleted, confirmed via grep: zero matches). Original Tasks 2/6 (rebuild that queue) are OBSOLETE — do not resurrect. Corrected model (user-confirmed):
> - **Tempo** = fully handled already. QC pass on a `purchaseMethod==='Vendor' && paymentMethod==='Tempo'` item auto-creates an AP bill (`recordVendorBillFromInbound`, built in pocket-work). Finance pays it later from the existing AP Aging page (already has a "Bayar Hutang Vendor" button — zero new UI needed). Pasar+Tempo items are a SEPARATE, pre-existing, already-correct path (`recordReconciliationSettlement`, scoped to `purchaseMethod==='Pasar'` only) — do NOT widen that filter, it would double-process Vendor items.
> - **Transfer** = posts immediately at sourcing laporan-submit time, same moment as Cash, just credits BCA (1-1200) instead of the pocket (1-1500). No queue, no finance approval step (per user: "normal2 aja").
> - **Location vs payment axis correction:** several sites the original table marked payment-based are actually LOCATION-based (they're about "did a vendor deliver this, so is price/vendor pre-known + does it skip the normal end-of-day Pasar cycle" — true for Vendor regardless of Cash/Tempo/Transfer): the sourcing skip-vendor-validation check, the pickup-vs-normal-buy branch, and the QC early-entry gate. Only the actual money-posting logic (pocket draw / BCA draw / AP-bill trigger) is payment-based. Steps below are updated in place to reflect this — read the code blocks in Tasks 3/4/5, not the original prose above them where they conflict.
