...
export default function ExpensesForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.transactionType === 'Kasbon') {
       ...
       // Auto-split if wallet is minus or insufficient
       const kasAmount = Math.max(0, Math.min(formData.amount, walletBalance))
       const reimbAmount = formData.amount - kasAmount
       ...
       if (reimbAmount > 0) {
         addReimbursement({
            ...
            userId: currentUser?.id || 'system',
            purchaseId: activePurchaseId,
            title: `${formData.category}: ${formData.description} (Auto-Talangan)`,
            amount: reimbAmount,
            description: formData.description,
            receiptUrl: formData.receiptUrl,
            status: 'Pending'
