...
export const recordDeliveryAndInvoice = async (deliveryId: string, invoiceId: string, invoiceTotal: number, cogsTotal: number, items: { productId: string, qty: number }[] = []) => {
  ...
  // Record Revenue (AR debit, Sales credit)
  ...
  // HPP is already recorded at purchase time (recordReconciliationSettlement / recordOnlinePurchase).
  // Here we only deduct physical stock.
  if (revSuccess) {
...
export const recordReconciliationSettlement = async (
  ...
  // 1. Settle Advance for Shop Cost (HPP) — record directly as HPP expense (like online purchases)
  if (actualShopCost > 0) {
    const settledFromAdvance = Math.min(actualShopCost, advanceAmount);
    const overspend = actualShopCost - settledFromAdvance;

    const credits: { accountCode: string; amount: number }[] = [
      { accountCode: '1-1500', amount: settledFromAdvance }
    ];
    // If sourcer overspent beyond advance, record the difference as AP (hutang)
    if (overspend > 0) {
      credits.push({ accountCode: '2-1000', amount: overspend });
    }
    ...
    await createAccountingEntry(
      ...
      'Purchase',
      purchaseId,
      [{ accountCode: '5-1000', amount: actualShopCost }],
      credits
    );
