...
export default function CourierExpensesPage() {
  ...
  const handleAddExpense = async () => {
    ...
    if (expenseType === 'Kasbon') {
       if (!receiptUrl) {
         setIsSubmitting(false)
         return toast.error("Bukti nota wajib diupload untuk reimbursement / kasbon!")
       }
       ...
       // Auto-split logic
       const kasAmount = Math.max(0, Math.min(amount, cashInHand))
       const reimbAmount = amount - kasAmount
       ...
       if (reimbAmount > 0) {
         await addReimbursement({
            ...
            date: new Date().toISOString(),
            userId: currentUser?.id || 'system',
            title: `${category}: ${description} (Auto-Talangan)`,
            amount: reimbAmount,
            description,
            receiptUrl,
            status: 'Pending'
       ...
       if (kasAmount > 0 && reimbAmount > 0) {
         ...
         toast.success("Laporan biaya (Potong Kas) dikirim ke Finance.")
       }
    }
