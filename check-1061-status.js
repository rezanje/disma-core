...
async function main() {
  const { data: ct, error: err1 } = await supabase
    .from('cash_transactions')
    .select('*')
    .eq('id', 'ct-bca-out-1061-mqkpwf18');
  console.log('Cash Tx:', ct, err1);

  const { data: je, error: err2 } = await supabase
    .from('journal_entries')
    .select('*')
    .eq('id', 'je-bca-out-1061-mqkpwf18');
  console.log('Journal Entry:', je, err2);
}
main();
