...
export default function Sidebar({ roleName }: SidebarProps) {
  ...
  const handleForceSync = () => {
    setIsSyncing(true)
    toast.loading("Membersihkan cache & memuat ulang...", { id: "sync" })
    // Step 1: Clear ALL localStorage caches
    clearAllOperationalCaches()
    // Step 2: Also clear any Zustand persisted state
    try { window.localStorage.removeItem('app-store'); } catch {}
    // Step 3: Hard reload — forces fresh init() with no stale data
    setTimeout(() => {
      ...
      <div className={cn(
        ...
        <button 
          ...
          className={cn(
            "p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/20 rounded-xl transition-all",
            isMinimized ? "w-10 h-10 flex items-center justify-center" : "",
            isSyncing && "text-emerald-500 bg-emerald-50"
          )}
        >
