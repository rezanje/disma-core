Create Codex pet row `running-right` for `anty`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A compact whole-body desktop pet called Anty: an obsidian and warm-white little helper with teal accents, soft rounded silhouette, tiny laptop-tile charm, attentive eyes, calm developer energy, no readable logos or text.. Preserve silhouette, face, palette, material, proportions, markings, and props.
