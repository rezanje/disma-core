...
export default function RejectionMonitorPage() {
  ...
  const totalMonetaryLoss = rejectedItems.reduce((sum, item) => {
    const product = products.find(p => p.id === item.productId)
    return sum + (item.qty * (product?.basePrice || 0))
  }, 0)
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         <Card className="liquid-card border-none bg-white/80 backdrop-blur-xl shadow-lg shadow-slate-100">
         ...
         <Card className="liquid-card border-none bg-rose-500 text-white shadow-lg shadow-rose-500/20">
            <CardContent className="p-6">
         ...
         <Card className="liquid-card border-none bg-slate-900 text-white shadow-lg shadow-slate-900/30">
            <CardContent className="p-6">
               <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">Estimasi Total Kerugian</p>
               <h3 className="text-3xl font-black text-rose-400">{formatRupiah(totalMonetaryLoss)}</h3>
            </CardContent>
         </Card>
      </div>
                     ...
                     <div className="p-6 flex-1 flex flex-col justify-between">
                        <div className="flex justify-between items-start">
                           ...
                           <div className="text-right">
                              ...
                              <p className="text-xs font-bold text-rose-500 mt-1">{formatRupiah(item.qty * (product?.basePrice || 0))}</p>
                           </div>
                        </div>
